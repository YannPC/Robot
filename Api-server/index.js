...
// Products data (moved from Angular to API)
const products = [
  {
    id: 1,
    description:
...
when making a post request if this for a single product
...
app.post("/api/cart", (req, res) => {
  ...
  cart.push(product);
  res.status(201).json({ message: `${product.name} added to cart`, cart });
});
*/

// when making the POST request if you want to send the entire array it's should work 
app.post("/api/cart", (req, res) => {
  const products = req.body; // expect an array

  if (!Array.isArray(products)) {
    return res.status(400).json({ error: "Invalid product data" });
  }

  cart = products; // replace server cart
